...
const deproque = new Deproque(writeFile, (args, executing) => executing.filter(path => path[0] === args[0]).length === 0);
...
const extendableKeyLock = new CustomLock();
writeMethods['extendableKeyLock'] = extendableKeyLock.addAccessor((args) => writeFile(...args), (newArg, queue) => queue.filter(comp => comp.arg[0] === newArg[0]).map(x => x.promise));
function randomString(n) {
    let acc = '';
    while (acc.length < n)
        acc += Math.random().toString(32).slice(2);
    return acc.slice(0, n);
}
const randomFileContents = new Array(fileCount * duplicateWrites);
const writeOrder = new Array(fileCount * duplicateWrites);
...
const contentIndexLookups = new Array(fileCount);
...
(async () => {
    ...
    for (const methodName in writeMethods) {
        ...
        const allWritePromises = new Array(fileCount * duplicateWrites);
        const t0 = Date.now();
        for (let i = 0, n = fileCount * duplicateWrites; i < n; i += 1)
